...
def get_features(df: pd.DataFrame, lag_params: Dict[str, Any]) -> Tuple[pd.DataFrame, pd.Timestamp]:
    ...
    Generates lagged features for the specified columns, useful for capturing
    temporal patterns in time-series forecasting. Missing values at the beginning
    of each lagged column are backfilled.
    ...
    Returns:
        DataFrame with original columns plus new lagged features.
    """
...
def train_model(
    ...
    Imports multiple model types which can be selected via the params dict.
    Model type matching is case-sensitive and ignores leading/trailing whitespace.
...
def compute_metrics(
    ...
    _______
    dict
        Dictionary with keys 'MAPE', 'MAE', 'RMSE" and their float values"""
...
def save_model(
    ...
    Uses model-specific serialisation when possible:
        - CatBoost: native .cbm format
        - XGBoost: native .xgb format
        - Other models: joblib .pkl format

    Args:
        model: Trained model instance.
        model_type: Type of model (for discerning save format).
    ...
    if model_type in ["catboost", "cb"]:
        # use native catboost serialisation
...
def load_model(
    ...
    Uses model-specific deserialisation:
    -CatBoost: native .cbm format
    -Other models: joblib .pkl format
